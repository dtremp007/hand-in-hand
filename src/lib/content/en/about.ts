import type { AboutContent } from '../types';

export const about: AboutContent = {
	mission: {
		title: 'The Two-Fold Focus',
		subtitle: 'of Our Mission',
		paragraphs: [
			'Hand-In-Hand Ministry exists in an effort to help men find freedom from Pornography. Our mission is to encourage individuals to establish a relationship with God, in which they CAN find freedom. Our focus is two-fold. The first part is that we understand the necessity of God, and more specifically, a relationship with Jesus Christ. If we have a lasting and meaningful relationship with Him, we know that no amount of pornography will have room to exist within our lives. The second part of our focus is to bring the hope that we have in Him to those around us.',
			'In essence, we want to see men place their hand in God\u2019s hand, trusting Him fully and never letting go. Simultaneously, we place our hand in the hand of a Seeker who struggles, and help them to find freedom by nurturing a committed relationship with our Lord Jesus Christ.',
			'As men, we can conquer the worldly lusts, by walking Hand-In-Hand with God, and we exemplify Christ\u2019s love by walking Hand-In-Hand with another whom we may help.'
		],
		diagram: {
			src: '/images/cross-english.webp',
			width: 771,
			height: 1000,
			alt: 'A wooden cross in a field, titled Our Cross. The horizontal beam reaches toward friends, family, and church on one side and dealing with issues on the other. The vertical beam is prayer, us talking to God, and Scripture, God talking to us.',
			caption: 'Luke 9:23'
		}
	},
	whoWeAre: {
		title: 'The Ones Behind This:',
		intro:
			'Without the following people, this vision may not have come to life, but first and foremost we must thank God for the way He has blessed us in a way that enables this ministry.',
		dedication:
			'Saying this, we would like to dedicate this website solely to Him. May this ministry be to His infinite glory!'
	},
	bob: {
		name: 'Bob Fife',
		role: 'Founder',
		bio: 'God has clearly revealed to me about the need for this ministry. It\u2019s one thing to have a struggle, it\u2019s another thing to know how to deal with it. The focus is important. I know what works. Hand in Hand with God is the only lasting solution. Once one receives real victory it\u2019s important to reach out and help someone who struggles. \u2018We should help others do what is right and build them up in the Lord\u2019 (Romans 15:2). My prayer is that churches around the world will utilize this amazing Ministry to help others and bring glory to God. Thank you in advance!'
	},
	acknowledgements: {
		title: 'ACKNOWLEDGEMENTS',
		intro:
			'We\u2019d like to acknowledge the following persons, who have all had a significant part in making this ministry happen.',
		items: [
			'Steinreich Bibleschule - The place this ministry began. We thank them genuinely for having given us the opportunity to begin this ministry among their students, hoping to leave an lasting impact!',
			'Henry Reimer - Research and Visionary',
			'Justin Loewen - Videographer and Media',
			'Brennan Plett - Tech, Visionary, and Originator'
		]
	},
	cta: {
		button: 'Contact Us'
	}
};
