import type { AboutContent } from '../types';

export const about: AboutContent = {
	mission: {
		title: 'EL DOBLE ENFOQUE',
		subtitle: 'DE NUESTRA MISI\u00d3N',
		paragraphs: [
			'Hand-In-Hand Ministry existe para ayudar a los hombres a liberarse de la pornograf\u00eda. Nuestra misi\u00f3n consiste en animar a las personas a establecer una relaci\u00f3n con Dios, en la que puedan encontrar la libertad. Nuestro enfoque es doble. La primera parte consiste en comprender la necesidad de Dios y, m\u00e1s concretamente, la necesidad de tener una relaci\u00f3n con Jesucristo. Si tenemos una relaci\u00f3n duradera y significativa con \u00c9l, sabemos que la pornograf\u00eda no tendr\u00e1 lugar en nuestras vidas. La segunda parte de nuestro enfoque consiste en transmitir la esperanza que tenemos en \u00c9l a quienes nos rodean.',
			'En esencia, queremos ver a los hombres poner su mano en la de Dios, confiando plenamente en \u00c9l y sin soltarla nunca. Al mismo tiempo, ponemos nuestra mano en la de quien lucha y lo ayudamos a encontrar la libertad al cultivar una relaci\u00f3n comprometida con nuestro Se\u00f1or Jesucristo.',
			'Como hombres, podemos vencer los deseos mundanos al caminar de la mano de Dios y mostrar el amor de Cristo ayudando a otros.'
		]
	},
	whoWeAre: {
		title: 'LOS QUE EST\u00c1N DETR\u00c1S DE ESTO:',
		intro:
			'Sin las siguientes personas, es posible que esta visi\u00f3n no se hubiera hecho realidad, pero, ante todo, debemos dar gracias a Dios por la forma en que nos ha bendecido, permiti\u00e9ndonos llevar a cabo este ministerio.',
		dedication:
			'Dicho esto, queremos dedicar este sitio web exclusivamente a \u00c9l. \u00a1Que este ministerio sea para Su gloria infinita!'
	},
	bob: {
		name: 'Bob Fife',
		role: 'Fundador',
		bio: 'Dios me ha revelado claramente la necesidad de este ministerio. Una cosa es tener una lucha y otra muy distinta es saber c\u00f3mo enfrentarse a ella. El enfoque es importante. S\u00e9 lo que funciona. \u00abDe la mano con Dios\u00bb es la \u00fanica soluci\u00f3n duradera. Una vez que se ha obtenido la verdadera victoria, es importante tender la mano y ayudar a quien est\u00e1 pasando por dificultades. \u00abDeber\u00edamos ayudar a otros a hacer lo que es correcto y edificarlos en el Se\u00f1or\u00bb (Romanos 15:2). Mi oraci\u00f3n es que las iglesias de todo el mundo utilicen este incre\u00edble ministerio para ayudar a otros y dar gloria a Dios. \u00a1Gracias de antemano!'
	},
	acknowledgements: {
		title: 'AGRADECIMIENTOS',
		intro:
			'Nos gustar\u00eda expresar nuestro agradecimiento a las siguientes personas, quienes han desempe\u00f1ado un papel fundamental en la puesta en marcha de este ministerio.',
		items: [
			'Steinreich Bibleschule: el lugar donde comenz\u00f3 este ministerio. Les agradecemos sinceramente por habernos dado la oportunidad de iniciar este ministerio entre sus estudiantes, \u00a1con la esperanza de dejar un impacto duradero!',
			'Henry Reimer - Investigador y visionario',
			'Justin Loewen - Camar\u00f3grafo y encargado de medios',
			'Brennan Plett - T\u00e9cnico, Visionario y Creador'
		]
	},
	cta: {
		button: 'Cont\u00e1ctanos'
	}
};
